...
const ServicePage = () => {
  ...
  return (
    <>
      <Head>
        <title>{service.title} - VTech Services</title>
        <meta name="description" content={service.content[0]?.paragraphs[0]?.substring(0, 160) || `Professional ${service.title} services from VTech`} />
      </Head>
      <div className="min-h-screen text-black">
        {service.content.map((section, index) => (
          <div key={index} className="flex flex-row py-[50px] gap-10 padding-x relative h-[80vh]">
            <Image src="/images/background.png" alt="Service background" fill className="object-cover z-0" priority />
            ...
            <div className="w-[50%] z-10 mt-[180px] relative">
              <Image src="/images/95.jpg" alt={`${service.title} illustration`} fill className="object-cover rounded-2xl z-10" />
            </div>
          </div>
        ))}
        ...
        <div className="py-[0px] padding-x flex flex-col gap-10">
          ...
          <div className="flex flex-row gap-10 h-[700px]">
            <div className="relative w-[50%] h-[700px]">
              <Image src="/images/95.jpg" alt={`${service.title} features`} fill className="object-cover rounded-2xl" />
            </div>
            <div className="grid grid-rows-3 grid-cols-2 gap-2 w-[50%]">
              {service.mobileCards.cards.map((card, index) => (
                ...
                >
                  <Icon icon={card.icon} width={35} height={35} color="gray" className="p-[10px] rounded-[10px] border-[1px] border-blue-500" />
