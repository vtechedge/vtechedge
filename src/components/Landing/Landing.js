...
const Landing = () => {
  ...
  const images = ["/images/landing-1.png", "/images/bg-2.jpg", "/images/bg-3.jpg"];
        ...
        <div className="absolute inset-0 z-10 flex items-center justify-center px-4">
          <div className="text-center space-y-6">
            ...
            <h1 className="text-3xl sm:text-4xl font-bold text-white leading-tight">
              {slideContent[currentSlide].subTitle.split(" ").map((word, index) => (
                <span key={index} className={index % 3 === 1 ? "text-blue-400" : ""}>
                  {word}{" "}
                </span>
              ))}
            </h1>

            <p className="text-lg text-gray-300 max-w-md mx-auto leading-relaxed">{slideContent[currentSlide].description}</p>
        ...
        <div className="grid grid-cols-2 gap-16 items-center w-full">
          ...
          <div className="space-y-8">
            <div className="space-y-6">
              ...
              <p className="text-xl text-gray-300 max-w-lg leading-relaxed">{slideContent[currentSlide].description}</p>
          ...
          <div className="relative">
            <img src={images[currentSlide]} alt={`Slide ${currentSlide + 1}`} className="w-full h-auto max-h-[600px] object-cover rounded-2xl shadow-2xl shadow-black/50" />
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex items-center gap-3 z-20">
        {slideContent.map((_, idx) => (
          <button
            ...
            className={`transition-all duration-300 ${idx === currentSlide ? "w-12 h-3 bg-blue-500 rounded-full" : "w-3 h-3 bg-white/40 hover:bg-white/60 rounded-full"}`}
          />
        ))}
      </div>
    </div>
